...
- Add text highlights, text overlays, whiteout rectangles, and optional replacement text
- Reorder, rotate, delete, insert blank, merge/import, and export selected pages
...
- **@dnd-kit**: organizer drag-and-drop reordering
...
- `src/components/*`: toolbar/sidebar/viewer/organizer/search/inspector/status UI
- `src-tauri/src/commands.rs`: Tauri command interface exposed to frontend
- `src-tauri/src/save_pipeline.rs`: safe save temp/replace/backup logic
...
- `Ctrl+Y` Redo
- `Ctrl+F` Search
- `Delete` Delete selected page(s) in organizer mode
- `Ctrl++` / `Ctrl+-` Zoom
...
- [x] **Phase 3** text overlays + whiteout + cover/replace workflow
...
- Text overlays are editable via inspector; drag/resize behavior is intentionally minimal for stability.
...
- optional replacement text overlay
- flattened into saved/exported output

This is intentionally labeled as **Whiteout / Cover & Replace** and is not claimed as guaranteed true in-place embedded text editing.
